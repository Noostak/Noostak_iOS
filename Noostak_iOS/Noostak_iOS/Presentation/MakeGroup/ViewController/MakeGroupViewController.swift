...
final class MakeGroupViewController: UIViewController, View {
    ...
    // MARK: Views
    private let rootView = MakeGroupView()
    
    init(reactor: MakeGroupReactor) {
    ...
    // MARK: bind
    func bind(reactor: MakeGroupReactor) {
        // MARK: Action
        // 사용자의 그룹 프로필 이미지 피커 버튼 탭
        ...
        rootView.nameTextField.rx.text
            ...
            .map { MakeGroupReactor.Action.inputName($0) }
        ...
        rootView.nameTextField.rx.deleteButtonTapped
            ...
            .map { MakeGroupReactor.Action.tapNameDeleteButton }
            .bind(to: reactor.action)
            .disposed(by: disposeBag)
...
extension MakeGroupViewController: PHPickerViewControllerDelegate {
